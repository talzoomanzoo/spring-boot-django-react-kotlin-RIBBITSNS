...
private UserRepository userRepository;
	...
	@GetMapping("/")
	public ResponseEntity<List<ListDto>> getAllLists(@RequestHeader("Authorization") String jwt) 
			throws ListException, UserException{
		
		User reqUser=userService.findUserProfileByJwt(jwt);
		List<ListModel> listModels= listService.findAllList();
	...
	public ResponseEntity<ListDto> addUserHandler1(@PathVariable Long userId,
		...
		updatedUser.setHasFollowedLists(ListUtil.isFollowedByReqList(updatedList, updatedUser));
